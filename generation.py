...
class Generation:
    ...
    def _add_organism_to_blocks(self, organism):
        ...
        radius = constants.RADIUS * organism.size
        if (organism.coordinates[0] < (x_coord - 1) * 10 + radius and
                x_coord - 1 > -1):
            self.blocks[x_coord - 1][y_coord].append(organism)
        if (organism.coordinates[0] > (x_coord + 1) * 10 - radius and
                x_coord + 1 < 10):
            self.blocks[x_coord + 1][y_coord].append(organism)
        if (organism.coordinates[1] < (y_coord - 1) * 10 + radius and
                y_coord - 1 > -1):
    ...
    def _move_everyone(self):
        ...
        smallest_alive = self.move(smallest, animate=True)
        if not smallest_alive:
            remove.add(smallest)
        for organism in self.organisms:
            if organism is not smallest:
                alive = self.move(organism)
                if not alive:
                    remove.add(organism)
        for dead in remove:
    ...
    def move(self, organism, animate=False):
        ...
        if animate:
            self.play_anim(organism, view_data)
    ...
    def play_anim(self, smallest, smallest_view):
        ...
        for idx in range(24):
            ...
            line = Line(smallest.coordinates[0] * RIGHT +
                        smallest.coordinates[1] * UP,
                        smallest.coordinates[0] * RIGHT +
                        smallest.coordinates[1] * UP +
